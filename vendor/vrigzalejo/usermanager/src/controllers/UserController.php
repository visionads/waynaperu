...
class UserController extends BaseController
{
    ...
    */
    public function getIndex()
    {
        ...
        $users = $emptyUsers->paginate(Config::get('usermanager::config.item-perge-page'));
        $datas['links'] = $users->links();
        $datas['users'] = $users;

        // ajax request : reload only content container
        if(Request::ajax()) {
            $html = View::make(Config::get('usermanager::views.users-list'), array('datas' => $datas))->render();

            return Response::json(array('html' => $html));
        }

        $this->layout = View::make(Config::get('usermanager::views.users-index'), array('datas' => $datas));
        $this->layout->title = trans('usermanager::users.titles.list');
        $this->layout->breadcrumb = Config::get('usermanager::breadcrumbs.users');
    }
    ...
    */
    public function getCreate()
    {
        ...
        $this->layout = View::make(Config::get('usermanager::views.user-create'), array('groups' => $groups, 'permissions' => $permissions));
        $this->layout->title = trans('usermanager::users.titles.new');
        $this->layout->breadcrumb = Config::get('usermanager::breadcrumbs.create_user');
    }
...
//                'username' => Input::get('username'),
            ...
            if(Config::get('usermanager::config.user-activation') === 'auto') {
                $user->attemptActivation($activationCode);
            } elseif(Config::get('usermanager::config.user-activation') === 'email') {
                $datas = array(
                    'code' => $activationCode,
                ...
                // send email
                Mail::queue(Config::get('usermanager::mails.user-activation-view'), $datas, function ($message) use ($user) {
                    $message->from(Config::get('usermanager::mails.email'), Config::get('usermanager::mails.contact'))
                            ->subject(Config::get('usermanager::mails.user-activation-object'));
    ...
    public function delete($userId)
    {
        try {
            if($userId !== Sentry::getUser()->getId()) {
                ...
                return Response::json(array('deletedUser' => false, 'message' => trans('usermanager::users.messages.remove-own-user'), 'messageType' => 'danger'));
            }
        } catch (\Cartalyst\Sentry\Users\UserNotFoundException $e) {
            return Response::json(array('deletedUser' => false, 'message' => trans('usermanager::users.messages.not-found'), 'messageType' => 'danger'));
        }

        return Response::json(array('deletedUser' => true, 'message' => trans('usermanager::users.messages.remove-success'), 'messageType' => 'success'));
    }
    ...
    public function putActivate($userId)
    {
        try {
            ...
            return Response::json(array('deletedUser' => false, 'message' => trans('usermanager::users.messages.activate-already'), 'messageType' => 'danger'));
        }

        return Response::json(array('deletedUser' => true, 'message' => trans('usermanager::users.messages.activate-success'), 'messageType' => 'success'));
    }
    ...
    public function getActivate($activationCode)
    {
        ...
        $this->layout = View::make(Config::get('usermanager::views.user-activation'), array('activated' => $activated, 'message' => $message));
    }
    ...
    */
    public function getShow($userId)
    {
        try {
            ...
            // ajax request : reload only content container
            if(Request::ajax()) {
                $html = View::make(Config::get('usermanager::views.user-informations'), array('user' => $user, 'throttle' => $throttle))->render();

                return Response::json(array('html' => $html));
            }

            $this->layout = View::make(Config::get('usermanager::views.user-profile'), array(
                'user' => $user,
                'throttle' => $throttle,
                'groups' => $groups,
            ...
            $this->layout = View::make(Config::get('usermanager::views.error'), array('message' => trans('usermanager::users.messages.not-found')));
        }
    }
